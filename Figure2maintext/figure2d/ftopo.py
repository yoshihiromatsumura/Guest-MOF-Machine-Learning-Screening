...
df = pd.read_excel('260529_ZnMOF_inclusion_35.xlsx', header=1, sheet_name=1, index_col=0)
...
fp = [AllChem.RDKFingerprint(mol) for mol in df['MOL'].values]
fps = []
for i in range(len(fp)):
    fps.append(np.array(list(fp[i].ToBitString()), int))
