...
fp = [AllChem.GetTopologicalTorsionFingerprint(mol) for mol in df['MOL'].values]
from collections import defaultdict
fps_tmp = []
keys = set()
for i in range(len(fp)):
    fps_tmp.append(fp[i].GetNonzeroElements())
    for key in fp[i].GetNonzeroElements():
        keys.add(key)
fps = defaultdict(list)
for fp0 in fps_tmp:
    for key in keys:
        if key in fp0:
            fps[key].append(fp0[key])
        else:
            fps[key].append(0)
