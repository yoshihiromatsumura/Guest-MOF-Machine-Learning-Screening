...
df = pd.read_excel('Dataset_X_35.xlsx', header=1, sheet_name=1, index_col=0)
dfx = pd.read_csv('to_soap_gc0.csv')
X = dfx.iloc[:,1:].values
y = df['Guest ratio  (from 1HNMR)'].values

from boruta import BorutaPy
rf = ExtraTreesRegressor(criterion='squared_error', 
                         max_depth=None, 
                         min_samples_split=2, 
                         min_samples_leaf=1, 
...
feat_selector = BorutaPy(rf, n_estimators='auto', verbose=2, 
                         perc=100, random_state=1, max_iter=5000)
feat_selector.fit(X, y)

selected = feat_selector.support_
print('num. selected: %d' % np.sum(selected))
print(selected)
XX = pd.DataFrame(dfx.iloc[:,1:].values,columns=dfx.iloc[:,1:].columns)
print(XX.columns[selected])
X_selected = XX[XX.columns[selected]]
...
print('SCORE with selected Features: %1.2f' % forest.score(X_selected, y))
...
def _lu_out(y0,y1,y2,y3,list_u):
    ltmp = pd.read_csv('file_subst_35').values
    labels = [str(j[0]) for j in ltmp]
    lbset = []
    for i in range(len(y0)):
        if y0[i] == y1:
            lbset.append(labels[i])
    print(*lbset,round(*y1,2),"(exp)",round(*y2,2),"(pred)",round(*y3,2),"(std)")
    list_u2 = []
